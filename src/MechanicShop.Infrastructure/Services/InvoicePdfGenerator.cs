    ...
    private Action<IContainer> BuildHeader(Invoice invoice) => header =>
    {
        header.Column(col =>
        {
            ...
            col.Item().Row(row =>
            {
                ...
                {
                    detailsCol.Item().Text($"INVOICE #{invoice.Id.ToString()[..8]}")
                        .FontSize(28)
                        .Bold()
        ...
        content.Column(col =>
        {
            ...
            col.Item().PaddingTop(30).Row(row =>
            {
                ...
                row.RelativeItem(1).Column(totalsCol =>
                {
                    ...
                    {
                        totalRow.RelativeItem().Text("Subtotal:").FontSize(11).FontColor(Colors.Grey.Darken3);
                        totalRow.RelativeItem().AlignRight().Text($"{invoice.Subtotal:C}").FontSize(11).FontColor(Colors.Grey.Darken3);
                    });

                    totalsCol.Item().PaddingVertical(5).Row(totalRow =>
                    {
                        totalRow.RelativeItem().Text("Tax:").FontSize(11).FontColor(Colors.Grey.Darken3);
