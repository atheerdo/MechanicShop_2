       ...
       : IRequestHandler<UpdateWorkOrderStateCommand, Result<Updated>>
{
    ...
    public async Task<Result<Updated>> Handle(UpdateWorkOrderStateCommand command, CancellationToken ct)
    {
        ...
        {
            _logger.LogError("WorkOrder with id: '{WorkOrder}' does not exist.", command.WorkOrderId);
        ...
        {
            _logger.LogError("State transition for WorkOrder Id '{WorkOrderId}` is not allowed before the work order scheduled start time.", command.WorkOrderId);
