...
public class AssignLaborCommandHandler( ILogger<AssignLaborCommandHandler> logger,
                                        ...
                                        : IRequestHandler<AssignLaborCommand, Result<Updated>>
{
    ...
    private readonly IWorkOrderPolicy _workOrderRuleService = workOrderRuleService;
    ...
    public async Task<Result<Updated>> Handle(AssignLaborCommand command, CancellationToken ct)
    {
        ...
        if (await _workOrderRuleService.IsLaborOccupied(command.LaborId, command.WorkOrderId, workOrder.StartAtUtc, workOrder.EndAtUtc))
        {
