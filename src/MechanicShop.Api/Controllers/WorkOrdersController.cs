...
public sealed class WorkOrdersController(ISender sender) : ApiController
{
    ...
    [MapToApiVersion("1.0")]
    public async Task<IActionResult> Get([FromBody] WorkOrderFilterRequest filters, [FromQuery] PageRequest pageRequest, CancellationToken ct)
    {
    ...
    [HttpPut("{workOrderId:guid}/assignLabor")]
    [Authorize(Policy = "ManagerOnly")]
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status404NotFound)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status400BadRequest)]
    ...
    [Authorize($"{nameof(Role.Manager)},{nameof(Role.Labor)}", Policy = "SelfScopedWorkOrderAccess")]
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status404NotFound)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status400BadRequest)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status500InternalServerError)]
    ...
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status400BadRequest)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status500InternalServerError)]
    [EndpointSummary("Update repair task by work order id.")]
    [EndpointDescription("Updates the current repair task of the specified work order. Only users with the Manager role are authorized.")]
    [EndpointName("UpdateRepairTasks")]
    [MapToApiVersion("1.0")]
    public async Task<IActionResult> UpdateRepairTasks(Guid worOrderId, ModifyRepairTaskRequest request, CancellationToken ct)
    {
        var command = new UpdateWorkOrderRepairTasksCommand(worOrderId, request.RepairTaskIds);
        ...
        CancellationToken ct)
    {
        ...
        var query = new GetDailyScheduleQuery(timeZone, scheduleDate, laborId);
