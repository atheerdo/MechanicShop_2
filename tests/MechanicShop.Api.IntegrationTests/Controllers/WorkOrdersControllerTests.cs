...
public class WorkOrdersControllerTests(WebAppFactory webAppFactory)
{
    ...
    public async Task RelocateWorkOrder_WithValidRequest_ShouldUpdateWorkOrder()
    {
        ...
        try
        {
            var response = await _client.PutAsJsonAsync($"/api/v1.0/workorders/{workOrder.Id}/relocate", request);
    ...
    public async Task AssignLabor_WithValidRequest_ShouldAssignLabor()
    {
        ...
        try
        {
          var response = await _client.PutAsJsonAsync($"/api/v1.0/workorders/{workOrder.Id}/assignLabor", request);
    ...
    public async Task AssignLabor_WithInvalidLaborId_ShouldReturnBadRequest()
    {
        ...
        var response = await _client.PutAsJsonAsync($"/api/v1.0/workorders/{Guid.NewGuid()}/assignLabor", request);
    ...
    public async Task UpdateWorkOrderState_WithValidRequest_ShouldUpdateState()
    {
        ...
        try
        {
           ...
           Assert.Equal(Domain.WorkOrders.Enums.WorkOrderState.InProgress, updated.State);
        }
        finally
        {
          await _context.WorkOrders
    ...
    [Fact]
    public async Task UpdateWorkOrderState_AsLabor_WithoutSelfScopedAccess_ShouldFail()
    {
        var token = await _client.GenerateTokenAsync(TestUsers.Labor02);
    ...
    public async Task DeleteWorkOrder_WithValidId_ShouldDeleteWorkOrder()
    {
        ...
        var exists = await _context.WorkOrders.AnyAsync(w => w.Id == workOrder.Id);
        Assert.False(exists);
    }
    ...
    public async Task DeleteWorkOrder_WithInvalidId_ShouldReturnNotFound()
    {
        ...
        var notFoundWorkOrderId = Guid.NewGuid();

        var response = await _client.DeleteAsync($"/api/v1.0/workorders/{notFoundWorkOrderId}");
        Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
    }
    ...
    public async Task AssignLabor_WithNonExistingWorkOrder_ShouldReturnNotFound()
    {
        ...
        var nonExistsWorkOrderId = Guid.NewGuid();
        ...
        var response = await _client.PutAsJsonAsync($"/api/v1.0/workorders/{nonExistsWorkOrderId}/assignLabor", request);
        Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
    }
    ...
    public async Task AssignLabor_WithoutAuthentication_ShouldReturnUnauthorized()
    {
        ...
        var response = await _client.PutAsJsonAsync($"/api/v1.0/workorders/{workOrderId}/assignLabor", request);
    ...
    public async Task UpdateWorkOrderState_WithInvalidWorkOrderId_ShouldReturnNotFound()
    {
        ...
        var response = await _client.PutAsJsonAsync($"/api/v1.0/workorders/{nonExistsWorkOrderId}/state", request);
        Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
    }
